...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	ret = 0;
        ...
        ret = blockValue * .50; // 50% to Masternode
    } else if (nHeight <= 57599 && nHeight >= 28800) {
	ret = blockValue * .60; // 60% to Masternode
    } else if (nHeight <= 86399 && nHeight >= 57600) {
	ret = blockValue * .70; // 70% to Masternode
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 86400) {
	ret = blockValue * .80; // 80% to Masternode
