...
class FantasyGoldDelegationPriv
{
public:
    FantasyGoldDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~FantasyGoldDelegationPriv()
    {
        if(m_pfDelegations)
...
FantasyGoldDelegation::FantasyGoldDelegation():
    priv(0)
{
    priv = new FantasyGoldDelegationPriv();
}
...
bool FantasyGoldDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool FantasyGoldDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool FantasyGoldDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> FantasyGoldDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void FantasyGoldDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool FantasyGoldDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string FantasyGoldDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool FantasyGoldDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
