...
FantasyGoldState::FantasyGoldState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = FantasyGoldState::openDB(_path + "/fantasygoldDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

FantasyGoldState::FantasyGoldState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute FantasyGoldState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, FantasyGoldTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createFantasyGoldAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
...
std::unordered_map<dev::Address, Vin> FantasyGoldState::vins() const // temp
{
...
void FantasyGoldState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* FantasyGoldState::vin(dev::Address const& _a) const
{
    return const_cast<FantasyGoldState*>(this)->vin(_a);
}

Vin* FantasyGoldState::vin(dev::Address const& _addr)
{
...
// void FantasyGoldState::commit(CommitBehaviour _commitBehaviour)
// {
...
void FantasyGoldState::kill(dev::Address _addr)
{
...
void FantasyGoldState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void FantasyGoldState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void FantasyGoldState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!FantasyGoldState::addressInUse(delegationsAddress)){
        FantasyGoldState::createContract(delegationsAddress);
        FantasyGoldState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
