#include <fantasygold/fantasygoldDGP.h>
...
void FantasyGoldDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool FantasyGoldDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule FantasyGoldDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t FantasyGoldDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t FantasyGoldDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t FantasyGoldDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t FantasyGoldDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool FantasyGoldDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void FantasyGoldDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void FantasyGoldDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void FantasyGoldDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void FantasyGoldDGP::createParamsInstance(){
...
dev::Address FantasyGoldDGP::getAddressForBlock(unsigned int blockHeight){
...
void FantasyGoldDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void FantasyGoldDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void FantasyGoldDGP::parseStorageOneUint64(uint64_t& value){
...
void FantasyGoldDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule FantasyGoldDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
