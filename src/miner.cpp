...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setFantasyGoldSchedule(fantasygoldDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = fantasygoldDGP.getBlockSize(nHeight);
    minGasPrice = fantasygoldDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = fantasygoldDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    FantasyGoldTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractFantasyGoldTX resultConverter;
    if(!convert.extractionFantasyGoldTransactions(resultConverter)){
    ...
    for(FantasyGoldTransaction fantasygoldTransaction : fantasygoldTransactions){
        txGas += fantasygoldTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + fantasygoldTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(fantasygoldTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, fantasygoldTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            fantasygoldDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = fantasygoldDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                fantasygoldDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                fantasygoldDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            fantasygoldDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            fantasygoldDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    FantasyGoldDelegation fantasygoldDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                fantasygoldDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = fantasygoldDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    fantasygoldDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    fantasygoldDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                fantasygoldDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(fantasygoldDelegations.GetDelegation(address, delegation) && FantasyGoldDelegation::VerifyDelegation(address, delegation))
                    {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "fantasygoldstake";
...
void StakeFantasyGolds(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
