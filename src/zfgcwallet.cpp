...
CzFGCWallet::CzFGCWallet(std::string strWalletFile) {
    ...
    //Check for old db version of storing zfgc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFGCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFGCSeed_deprecated()) {
                    LogPrintf("%s: Updated zFGC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfgc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfgc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFGCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount) {
    ...
    if (fResetCount)
        walletdb.WriteZFGCCount(nCountLastUsed);
    else if (!walletdb.ReadZFGCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFGCWallet::Lock() {
    seedMaster = 0;
}

void CzFGCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose) {
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFGCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd) {
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFGC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFGCWallet::LoadMintPoolFromDB() {
...
void CzFGCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes) {
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFGCWallet::GetState(int& nCount, int& nLastGenerated) {
    nCount = this->nCountLastUsed + 1;
    nLastGenerated = mintPool.CountOfLastGenerated();
}

//Catch the counter up with the chain
void CzFGCWallet::SyncWithChain(bool fGenerateMintPool) {
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfgcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFGCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom) {
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFGC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfgcTracker which also adds to database
    pwalletMain->zfgcTracker->Add(dMint, true);
...
void CzFGCWallet::SeedToZFGC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key) {
...
uint512 CzFGCWallet::GetZerocoinSeed(uint32_t n) {
...
void CzFGCWallet::UpdateCount() {
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZFGCCount(nCountLastUsed);
}

void CzFGCWallet::GenerateDeterministicZFGC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly) {
...
void CzFGCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint) {
...
bool CzFGCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint) {
