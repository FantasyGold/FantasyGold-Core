...
public:
    ...
    bool isUnspentAddress(const std::string &fantasygoldAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == fantasygoldAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_fantasygoldDelegation.ExistDelegationContract() ? m_fantasygoldDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_fantasygoldDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_fantasygoldDelegation.ExistDelegationContract() ? m_fantasygoldDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_fantasygoldDelegation.VerifyDelegation(address, delegation);
    ...
    FantasyGoldDelegation m_fantasygoldDelegation;
};
