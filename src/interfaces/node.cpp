...
public:
    ...
    void getGasInfo(uint64_t& blockGasLimit, uint64_t& minGasPrice, uint64_t& nGasPrice) override
    {
        ...
        blockGasLimit = fantasygoldDGP.getBlockGasLimit(::ChainActive().Height());
        minGasPrice = CAmount(fantasygoldDGP.getMinGasPrice(::ChainActive().Height()));
    ...
    WWalletCreationStatus createWallet(const SecureString& passphrase, uint64_t wallet_creation_flags, const std::string& name, std::string& error, std::vector<std::string>& warnings, std::unique_ptr<Wallet>& result) override
    {
