...
#include <fantasygold/storageresults.h>
...
using ExtractFantasyGoldTX = std::pair<std::vector<FantasyGoldTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // fantasygold
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashFantasyGoldRoot,
    const dev::h256& oldHashStateRoot, const std::vector<FantasyGoldTransaction>& transactions);
...
class FantasyGoldTxConverter{

public:

    FantasyGoldTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionFantasyGoldTransactions(ExtractFantasyGoldTX& fantasygoldTx);
...
private:
    ...
    FantasyGoldTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<FantasyGoldTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
