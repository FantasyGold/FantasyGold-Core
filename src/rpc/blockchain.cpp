...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // fantasygold
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // fantasygold

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // fantasygold
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // fantasygold
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // fantasygold
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // fantasygold
...
UniValue transactionReceiptToJSON(const FantasyGoldTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if fantasygoldd is started with -deprecatedrpc=size\n"
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination fantasygoldSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(fantasygoldSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&fantasygoldSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The fantasygold address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    FantasyGoldDelegation fantasygoldDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!fantasygoldDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = fantasygoldDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The fantasygold address string for staker"},
                },
                RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!fantasygoldDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = fantasygoldDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of fantasygold addresses",
                                    {{RPCResult::Type::STR, "address", "fantasygold address"}}},
                            }},
