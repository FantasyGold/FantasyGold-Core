...
#include "ui_zfgccontroldialog.h"
...
std::list<std::string> ZFgcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFgcControlDialog::listMints;

ZFgcControlDialog::ZFgcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFgcControlDialog),
    model(0) {
    ui->setupUi(this);
    listMints.clear();
...
ZFgcControlDialog::~ZFgcControlDialog() {
    delete ui;
}

void ZFgcControlDialog::setModel(WalletModel *model) {
    this->model = model;
    updateList();
}

//Update the tree widget
void ZFgcControlDialog::updateList() {
...
void ZFgcControlDialog::updateSelection(QTreeWidgetItem* item, int column) {
...
void ZFgcControlDialog::updateLabels() {
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFgc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFgcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFgcControlDialog::GetSelectedMints() {
...
void ZFgcControlDialog::ButtonAllClicked() {
    ui->treeWidget->blockSignals(true);
