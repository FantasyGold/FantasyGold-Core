#include <qt/fgctoken.h>
#include <qt/forms/ui_fgctoken.h>
...
void FGCToken::setModel(WalletModel *_model)
{
    m_model = _model;
...
void FGCToken::setClientModel(ClientModel *_clientModel)
{
...
void FGCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void FGCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void FGCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
