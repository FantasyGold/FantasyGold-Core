...
{
    int maxHalvings = 7;
    CAmount nInitialSubsidy = 5 * COIN;

    CAmount nPreviousSubsidy = nInitialSubsidy * 2.5 ; // for height == LastPoWBlock + 1
    BOOST_CHECK_EQUAL(nPreviousSubsidy, nInitialSubsidy * 2.5);
    for (int nHalvings = 0; nHalvings < maxHalvings; nHalvings++) {
        ...
        BOOST_CHECK_EQUAL(nSubsidy, nPreviousSubsidy / 2.5);
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    BOOST_CHECK_EQUAL(nSum, 21000000025000000ULL);
}
