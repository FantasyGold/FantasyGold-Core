...
void checkResult(bool isCreation, std::vector<FantasyGoldTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    FantasyGoldTxConverter converter(transaction, NULL);
    ExtractFantasyGoldTX fantasygoldTx;
    BOOST_CHECK(converter.extractionFantasyGoldTransactions(fantasygoldTx));
    std::vector<FantasyGoldTransaction> result = fantasygoldTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(fantasygoldtxconverter_tests, TestingSetup)
