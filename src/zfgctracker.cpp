...
CzFGCTracker::CzFGCTracker(std::string strWalletFile) {
...
CzFGCTracker::~CzFGCTracker() {
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFGCTracker::Init() {
...
bool CzFGCTracker::Archive(CMintMeta& meta) {
...
bool CzFGCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic) {
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFGCTracker::Get(const uint256 &hashSerial) {
...
CMintMeta CzFGCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin) {
...
bool CzFGCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFGCTracker::GetSerialHashes() {
...
CAmount CzFGCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const{
    CAmount nTotal = 0;
...
CAmount CzFGCTracker::GetUnconfirmedBalance() const{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFGCTracker::GetMints(bool fConfirmedOnly) const{
...
bool CzFGCTracker::HasMintTx(const uint256& txid) {
    for (auto it : mapSerialHashes) {
...
bool CzFGCTracker::HasPubcoin(const CBigNum &bnValue) const{
...
bool CzFGCTracker::HasPubcoinHash(const uint256& hashPubcoin) const{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
            return true;
...
bool CzFGCTracker::HasSerial(const CBigNum& bnSerial) const{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFGCTracker::HasSerialHash(const uint256& hashSerial) const{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFGCTracker::UpdateZerocoinMint(const CZerocoinMint& mint) {
...
bool CzFGCTracker::UpdateState(const CMintMeta& meta) {
    CWalletDB walletdb(strWalletFile);
...
void CzFGCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived) {
    CMintMeta meta;
...
void CzFGCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived) {
    CMintMeta meta;
...
void CzFGCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid) {
...
void CzFGCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin) {
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
    meta.isUsed = false;
...
void CzFGCTracker::RemovePending(const uint256& txid) {
    uint256 hashSerial;
...
bool CzFGCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint) {
...
std::set<CMintMeta> CzFGCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus) {
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzfgc from DB\n", __func__, listDeterministicDB.size());
    }
