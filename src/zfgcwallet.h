...
#ifndef FANTASYGOLD_ZFGCWALLET_H
#define FANTASYGOLD_ZFGCWALLET_H
...
public:
    CzFGCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFGC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFGC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FANTASYGOLD_ZFGCWALLET_H
