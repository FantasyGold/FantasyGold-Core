...
FantasyGoldDelegation& GetFantasyGoldDelegation()
{
    static FantasyGoldDelegation fantasygoldDelegation;
    return fantasygoldDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, CValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        FantasyGoldDelegation& fantasygoldDelegation = GetFantasyGoldDelegation();
        if(!fantasygoldDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = fantasygoldDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    FantasyGoldDelegation& fantasygoldDelegation = GetFantasyGoldDelegation();
    bool ret = fantasygoldDelegation.GetDelegation(address, delegation);
    if(ret) ret &= fantasygoldDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
