...
const char * const BITCOIN_CONF_FILENAME = "fantasygold.conf";
const char * const BITCOIN_PID_FILENAME = "fantasygoldd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FantasyGold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FantasyGold
    // Mac: ~/Library/Application Support/FantasyGold
    // Unix: ~/.fantasygold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FantasyGold";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FantasyGold";
#else
    // Unix
    return pathRet / ".fantasygold";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("FantasyGold Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The FantasyGold Core developers";
    }
