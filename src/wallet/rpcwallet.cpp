...
static UniValue createwallet(const JSONRPCRequest& request)
{
    ...
    WalletCreationStatus status = CreateWallet(*g_rpc_node->chain, passphrase, flags, request.params[0].get_str(), error, create_warning, wallet);
    switch (status) {
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getaddressinfo",
                ...
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The fantasygold address to get the information of."},
                },
                RPCResult{
            "{\n"
