...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (vin == CTxIn()) {
            ...
            LogPrint("masternode", "dseg - Sent %d Masternode entries to %s\n", nInvCount, pfrom->vin.prevout.hash.ToString());
        }
    }
    /*
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 57806) return;
        } else if (addr.GetPort() == 57806)
            return;
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 10,000 FantasyGold tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
