...
#if defined(NDEBUG)
# error "FantasyGold cannot be compiled without assertions."
#endif
...
std::unique_ptr<FantasyGoldState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = fantasygoldDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        ...
        FantasyGoldTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractFantasyGoldTX resultConverter;
        if(!converter.extractionFantasyGoldTransactions(resultConverter)){
        ...
        std::vector<FantasyGoldTransaction> fantasygoldTransactions = resultConverter.first;
        std::vector<EthTransactionParams> fantasygoldETP = resultConverter.second;
        ...
        for(FantasyGoldTransaction fantasygoldTransaction : fantasygoldTransactions){
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, fantasygoldTransaction.gasPrice());
            } else {
                txMinGasPrice = fantasygoldTransaction.gasPrice();
            }
            VersionVM v = fantasygoldTransaction.getVersion();
            if(v.format!=0)
            ...
            if(fantasygoldTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(fantasygoldTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(fantasygoldTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += fantasygoldTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)fantasygoldTransaction.gasPrice() < minGasPrice)
        ...
        if(!CheckMinGasPrice(fantasygoldETP, minGasPrice))
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-small-gasprice");

        if(count > fantasygoldTransactions.size())
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // fantasygold
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // fantasygold
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    {
        callTransaction = FantasyGoldTransaction(nAmount, 1, dev::u256(gasLimit), opcode, dev::u256(0));
    }
    else
    {
        callTransaction = FantasyGoldTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<FantasyGoldTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path fantasygoldDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(fantasygoldDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(fantasygoldDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(FantasyGoldTransaction& tx : txs){
        //validate VM version
...
bool FantasyGoldTxConverter::extractionFantasyGoldTransactions(ExtractFantasyGoldTX& fantasygoldtx){
    ...
    std::vector<FantasyGoldTransaction> resultTX;
    ...
    fantasygoldtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool FantasyGoldTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool FantasyGoldTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
FantasyGoldTransaction FantasyGoldTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    FantasyGoldTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = FantasyGoldTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = FantasyGoldTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t FantasyGoldTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // fantasygold
    FantasyGoldDGP fantasygoldDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setFantasyGoldSchedule(fantasygoldDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = fantasygoldDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = fantasygoldDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = fantasygoldDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // fantasygold
...
///////////////////////////////////////////////////////////////////////////////////////// fantasygold
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            FantasyGoldTxConverter convert(tx, &view, &block.vtx, contractflags);
            ...
            if(!CheckMinGasPrice(resultConvertFantasyGoldTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertFantasyGoldTX.first, blockGasLimit, pindex->pprev);
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // fantasygold
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // fantasygold
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // fantasygold
            globalState->setRootUTXO(oldHashUTXORoot); // fantasygold
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // fantasygold
        uint32_t sizeBlockDGP = fantasygoldDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for FantasyGold PoS we need to sync this after every block to ensure txdb is populated for
