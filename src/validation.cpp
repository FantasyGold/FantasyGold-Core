...
const std::string strMessageMagic = "FantasyGold Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            FantasyGoldTxConverter converter(tx, NULL);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // fantasygold
    FantasyGoldDGP fantasygoldDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setFantasyGoldSchedule(fantasygoldDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = fantasygoldDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = fantasygoldDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = fantasygoldDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // fantasygold
...
///////////////////////////////////////////////////////////////////////////////////////// fantasygold
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            FantasyGoldTxConverter convert(tx, &view, &block.vtx);
            ...
            if (fLogEvents && !fJustCheck)
            {
                for(size_t k = 0; k < resultConvertFantasyGoldTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertFantasyGoldTX.first[k].from(), resultConvertFantasyGoldTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 8800 && !Params().GetConsensus().fPoSNoRetargeting) {
