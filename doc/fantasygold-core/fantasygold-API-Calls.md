# FantasyGold API call list (as of version 2.0.0)

Common operations
-----------------

### Listing my FantasyGold addresses

Listing the FantasyGold addresses in your wallet is easily done via
...
| `stop` |   | Stop fgc server. | N |
| Generating |
| `getgenerate` |   | **PoW Only** Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or fgc.conf setting gen) It can also be set with the setgenerate call. | N |
