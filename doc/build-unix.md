UNIX BUILD NOTES
====================
Some notes on how to build FantasyGold Core in Unix.
...
memory available when compiling FantasyGold Core. On systems with less, gcc can be
...
You need to build boost manually, and if it's not in standard library paths, you need to add `/path/to/boost/lib` into `LD_LIBRARY_PATH` env when building FantasyGold.
...
Security
--------
To help make your FantasyGold Core installation more secure by making certain attacks impossible to
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, FantasyGold Core may be compiled in
