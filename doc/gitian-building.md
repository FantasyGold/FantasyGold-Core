...
*Setup instructions for a Gitian build of FantasyGold Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FantasyGold
...
- [Building FantasyGold Core](#building-fantasygold-core)
...
Clone the git repositories for fantasygold and Gitian.
...
```bash
sudo cp /home/debian/fantasygold/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the fantasygold repository under 'Fetch and create inputs' to install sources which require
...
To build FantasyGold Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fantasygold repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fantasygold/.git/
...
For example:
```bash
URL=https://github.com/laanwj/fantasygold.git
...
./bin/gbuild --commit fantasygold=${COMMIT} --url fantasygold=${URL} ../fantasygold/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-fantasygold:
./bin/gbuild --commit fantasygold=${COMMIT},cpp-eth-fantasygold=${ETHCOMMIT} --url fantasygold=${URL},cpp-eth-fantasygold=${ETHURL} ../fantasygold/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the fantasygold git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fantasygold/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/FantasyGold/FantasyGold-Core-detached-sigs.git

BTCPATH=/some/root/path/fantasygold
SIGPATH=/some/root/path/fantasygold-detached-sigs

./bin/gbuild --url fantasygold=${BTCPATH},signature=${SIGPATH} ../fantasygold/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fantasygold-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fantasygold-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fantasygold-osx-build.assert
```
...
[fantasygoldproject/gitian.sigs](https://github.com/fantasygold/gitian.sigs/) repository, or if that's not possible create a pull
