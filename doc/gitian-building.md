...
*Setup instructions for a gitian build of FantasyGold using a Debian VM or physical system.*
...
- [Building FantasyGold](#building-fantasygold)
...
Clone the git repositories for fantasygold and gitian and then checkout the fantasygold version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the fantasygold repository
...
To build FantasyGold (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the fantasygold repository.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fantasygold-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fantasygold-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/fantasygold-build.assert
```
