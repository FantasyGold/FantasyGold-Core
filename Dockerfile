...
COPY depends /FantasyGold/depends
ENV SDK_PATH=/FantasyGold/depends/SDKs/
RUN dpkg --add-architecture i386 \
  ...
  && wget https://github.com/phracker/MacOSX-SDKs/releases/download/10.13/MacOSX10.11.sdk.tar.xz -P /FantasyGold/depends/SDKs/ \
  && tar xJf /FantasyGold/depends/SDKs/MacOSX10.11.sdk.tar.xz -C /FantasyGold/depends/SDKs/ \
  && rm /FantasyGold/depends/SDKs/MacOSX10.11.sdk.tar.xz \
  && wget https://bootstrap.pypa.io/get-pip.py \
  && python get-pip.py \
  && rm get-pip.py \
  && pip install ez_setup==0.9 \
  && make -C /FantasyGold/depends HOST=arm-linux-gnueabihf \
  && make -C /FantasyGold/depends HOST=i686-pc-linux-gnu \
  && make -C /FantasyGold/depends HOST=i686-w64-mingw32 \
  && make -C /FantasyGold/depends HOST=x86_64-unknown-linux-gnu \
  && make -C /FantasyGold/depends HOST=x86_64-w64-mingw32

FROM dependencies AS base
COPY autogen.sh /FantasyGold/
COPY build-aux /FantasyGold//build-aux
COPY configure.ac /FantasyGold/
COPY contrib /FantasyGold//contrib
COPY Makefile.am /FantasyGold/
COPY pkg.m4 /FantasyGold/
COPY qa /FantasyGold/qa
COPY share /FantasyGold/share
COPY src /FantasyGold/src
WORKDIR /FantasyGold

FROM base AS arm32
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
COPY --from=dependencies /FantasyGold/depends/arm-linux-gnueabihf $(pwd)/depends/arm-linux-gnueabihf
RUN ./autogen.sh \
  && ./configure \
  --enable-zmq \
...
ENV CXXFLAGS="-Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-narrowing"
COPY --from=dependencies /FantasyGold/depends/i686-pc-linux-gnu $(pwd)/depends/i686-pc-linux-gnu
RUN ./autogen.sh \
  && ./configure \
  --enable-zmq \
  --with-pic \
...
COPY --from=dependencies /FantasyGold/depends/x86_64-unknown-linux-gnu $(pwd)/depends/x86_64-unknown-linux-gnu
RUN ./autogen.sh \
  && ./configure \
  --enable-zmq \
  --with-pic \
...
COPY --from=dependencies /FantasyGold/depends/i686-w64-mingw32 $(pwd)/depends/i686-w64-mingw32
COPY autogen.sh .
...
COPY --from=dependencies /FantasyGold/depends/x86_64-w64-mingw32 $(pwd)/depends/x86_64-w64-mingw32
COPY autogen.sh .
COPY build-aux ./build-aux
COPY configure.ac .
COPY contrib ./contrib
...
LABEL name="FantasyGold Docker Builds"
RUN mkdir /release
