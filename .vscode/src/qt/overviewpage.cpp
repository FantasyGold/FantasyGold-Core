...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeFantasyGoldAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFantasyGoldAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeFantasyGoldAmount = strAnonymizeFantasyGoldAmount.remove(strAnonymizeFantasyGoldAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFantasyGoldAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeFantasyGoldAmount * COIN) nMaxToAnonymize = nAnonymizeFantasyGoldAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeFantasyGoldAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeFantasyGoldAmount));
        strAnonymizeFantasyGoldAmount = strAnonymizeFantasyGoldAmount.remove(strAnonymizeFantasyGoldAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFantasyGoldAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeFantasyGoldAmount == 0) {
